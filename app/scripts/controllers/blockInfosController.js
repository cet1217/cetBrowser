	...
	var web3 = $rootScope.web3;
        ...
        $scope.init = function()
        {
            ...
            if($scope.blockId!==undefined) {
                ...
                getBlockInfos()
                    .then(function(result){
                        var number = web3.cet.blockNumber;
                    ...
                    if($scope.blockNumber!==undefined){
                        var info = web3.cet.getBlock($scope.blockNumber);
                        if(info!==undefined){
            ...
            function getBlockInfos() {
                ...
                web3.cet.getBlock($scope.blockId,function(error, result) {
                    if(!error) {
        ...
        // parse transactions
        $scope.transactions = []
        web3.cet.getBlockTransactionCount($scope.blockId, function(error, result){
          var txCount = result

          for (var blockIdx = 0; blockIdx < txCount; blockIdx++) {
            web3.cet.getTransactionFromBlock($scope.blockId, blockIdx, function(error, result) {
