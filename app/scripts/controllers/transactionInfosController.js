        ...
        $scope.init=function()
        {
            ...
            if($scope.txId!==undefined) { // add a test to check if it match tx paterns to avoid useless API call, clients are not obliged to come from the search form...
                ...
                getTransactionInfos()
                    ...
                    $scope.cetValue = result.value.c[0] / 10000; 
                    $scope.txprice = (result.gas * result.gasPrice)/1000000000000000000 + " CET";
            ...
            function getTransactionInfos(){
                ...
                web3.cet.getTransaction($scope.txId,function(error, result) {
                    if(!error){
